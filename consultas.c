...
int main(int argc, char **argv)
{
	...
	conn = mysql_real_connect (conn, "localhost","root", "mysql", "Partida",0, NULL, 0);
	if (conn==NULL) {
	...
	if (row == NULL)
		...
		while (row !=NULL) {			
			printf ("La puntuación máxima histórica del jugador es: %d\n", row[0]);
	}
	...
	if (row == NULL)
		...
		while (row !=NULL) {			
			printf ("El número de partidas jugadas por dicho jugador es de: %d partidas.\n", row[0]);
	}
		mysql_close (conn);
		exit(0);
}
